        ...
        with QMutexLocker(self._mutex):
            return {
                level_id: {
                    ...
                    'entry_price': float(level.entry_price) if level.entry_price is not None else 0.0,
                    'exit_price': float(level.exit_price) if level.exit_price is not None else 0.0,
                    'shares': level.shares,
